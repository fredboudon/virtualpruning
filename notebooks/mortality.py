...
def pruned_gu_mortality_2022(diameter, intensity, severity, Zeta_min):
...
def unpruned_gu_mortality_2022(diameter, Zeta_min):
    intercept = -1.597 
...
## Occurence
def unpruned_gu_daughter_mortality_occurence_2022(nb_daughter_gus): 
    intercept = - 7.641
...
def unpruned_gu_daughter_mortality_intensity_2022(nb_daughter_gus): 
    return poisson_realization(poisson_proba_from_latent(-0.64), min(3,nb_daughter_gus-1))+1

def pruned_gu_daughter_mortality_occurence_2022(nb_daughter_gus):
    intercept = - 2.362
...
## Intensity
def pruned_gu_daughter_mortality_intensity_2022(nb_daughter_gus):
    intercept = -1.514
...
def gu_mortalities_post_pruning(mtg, Zeta_min = None, intensity = None, pruningdate = date(2021,2,24), inplace = False):
   ...
   for vid in terminals:
        if vid in prunedgus:
            ...
            occurence = pruned_gu_mortality_2022(get_gu_diameter(mtg, vid), intensity, severity, Zeta_min.get(vid,0))
            if occurence:
                toremove.append(vid)
        else:
            occurence = unpruned_gu_mortality_2022(get_gu_diameter(mtg, vid), Zeta_min.get(vid,0))
            if occurence:
                toremove.append(vid)
...
def gu_mortalities_post_regrowth(mtg, newids, inplace = False):
   ...
   for vid in terminals:
        if vid in prunedgus:
            occurence = pruned_gu_daughter_mortality_occurence_2022(mtg.nb_children(vid))
            if occurence:
                daughters = mg.get_children(mtg,vid)
                nbdeaddaughters = pruned_gu_daughter_mortality_intensity_2022(mtg.nb_children(vid))
                toremove += np.random.choice(daughters, size=nbdeaddaughters, replace=False)
        else:
            occurence = unpruned_gu_daughter_mortality_occurence_2022(mtg.nb_children(vid))
            if occurence:
                daughters = mg.get_children(mtg,vid)
                nbdeaddaughters = unpruned_gu_daughter_mortality_intensity_2022(mtg.nb_children(vid))
                toremove += np.random.choice(daughters, size=nbdeaddaughters, replace=False)
...
def gu_mortality_post_regrowth(mtg, motherid, nb_daughter_gus):
    ...
    if motherid in mtg.property('pruned'):
        occurence = pruned_gu_daughter_mortality_occurence_2022(mtg.nb_children(vid))
        if occurence:
            nbdeaddaughters = pruned_gu_daughter_mortality_intensity_2022(nb_daughter_gus)
            return nb_daughter_gus - nbdeaddaughters
    else:
        occurence = unpruned_gu_daughter_mortality_occurence_2022(mtg.nb_children(vid))
        if occurence:
            nbdeaddaughters = unpruned_gu_daughter_mortality_intensity_2022(nb_daughter_gus)
            return nb_daughter_gus - nbdeaddaughters
